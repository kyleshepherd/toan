<script lang="ts">
  ...
  import { matrices, getMatrixRow, type MatrixName } from "$lib/data/matrices";
  ...
  function findMatchingDescription(encounterName: string, descriptions: readonly string[]): string | null {
  ...
  function handleSubmit() {
    ...
    const matchedDescription = findMatchingDescription(
      encounter.name,
      matrix.descriptions
    );
...
<div class="container mx-auto max-w-4xl p-4 space-y-6">
  ...
  <Card.Root class="max-w-md mx-auto">
    ...
    <Card.Content class="space-y-4">
      <div class="space-y-2">
        <Label for="encounter-card">Encounter Card Number <span class="text-destructive">*</span></Label>
        <Input
          id="encounter-card"
          type="number"
          min="0"
    ...
    <Card.Root>
      ...
      <Card.Content class="overflow-x-auto">
        <Table.Root>
          <Table.Header>
            <Table.Row>
              <Table.Head class="sticky left-0 z-10 bg-background"></Table.Head>
              {#each result.matrixData.actions as action}
                <Table.Head class="text-center text-xs min-w-[70px]">{action}</Table.Head>
              {/each}
            </Table.Row>
          </Table.Header>
          <Table.Body>
            {#each result.matrixData.descriptions as description}
              ...
              <Table.Row class="group {isHighlighted ? 'bg-primary/20 font-semibold' : ''}">
                <Table.Cell class="sticky left-0 z-10 font-medium {isHighlighted ? '!bg-accent' : '!bg-background group-hover:!bg-muted'}">
                  {description}
                </Table.Cell>
                {#each result.matrixData.actions as action}
